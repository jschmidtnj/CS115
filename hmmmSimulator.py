    ...
    if opcode == "halt":  
        ...
        while user_input == "" \
          or  (not (user_input.isdigit() \
            or (user_input[0] == '-' and user_input[1:].isdigit()))) \
          or not valid_integer(int(user_input)):
            print("\n\nIllegal user_input: number must be in [-32768,32767]")
            user_input = input("Enter number (q to quit): ")
            if user_input == "q" :
                sys.exit()
        register[args[0]] = int(user_input) 
