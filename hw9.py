...
#print(letterScoreLoop('f', scrabbleScores))
...
#print(wordScoreLoop('test', scrabbleScores))
...
def wordsWithScoreLambda(dct, scores):
    ...
    return map(lambda x: [dct[x], wordScore(dct[x], scores)], range(len(dct)))

#print(wordsWithScoreLambda(['python', 'is', 'awesome'], scrabbleScores))
...
def wordsWithScoreLoop(dct, scores):
    ...
    for x in range(len(dct)):
        result += [[dct[x], wordScore(dct[x], scores)]]
    return result
